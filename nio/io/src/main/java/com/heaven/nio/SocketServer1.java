...
public class SocketServer1 {
    ...
    public static void main(String args[]) throws IOException {
        ...
        serverChannel.configureBlocking(false); //非阻塞模式通道
        ServerSocket serverSocket = serverChannel.socket(); //获取与此通道关联的服务器套接字
        serverSocket.setReuseAddress(true); //启用 SO_REUSEADDR 套接字选项
        ...
        try {
            while(true){
                ...
                if(selector.select(100) == 0){ //选择一组键，其相应的通道已为 I/O 操作准备就绪
                    //================================================
                    //      这里视业务情况，可以做一些然并卵的事情
                    //================================================
                    continue;
    ...
    private static void readSocketChannel(SelectionKey readyKey) throws Exception {
        ...
        if(message.indexOf("over") != -1){
            ...
            contextBytes.clear();
            SocketServer1.LOGGER.info("端口:" + resourcePort + "客户端发来的信息======message : " + message);
            ...
            SocketServer1.LOGGER.info("端口:" + resourcePort + "客户端信息还未接受完，继续接受======message : " + message);
